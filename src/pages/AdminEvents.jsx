...
import { Archive, CalendarDays, ChevronDown, Copy, MapPin, Plus } from 'lucide-react'
import AdminHeader from '../components/AdminHeader'
import { listEvents, updateEventStatus } from '../services/api'
...
export default function AdminEvents() {
  ...
  return <><AdminHeader /><main className="mx-auto max-w-7xl space-y-8 p-4 sm:p-6">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-[.2em] text-brand-800">Panel del organizador</p><h1 className="mt-1 text-3xl font-extrabold">Mis eventos</h1><p className="mt-1 text-slate-500">Crea, configura y supervisa cada competencia.</p></div><div className="flex flex-wrap gap-2"><a className="btn-secondary inline-flex items-center gap-2" href={events[0] ? `/admin/eventos/clonar/${events[0].id}` : '/admin/eventos/nuevo'}><Copy className="size-4" />Clonar evento anterior</a><a className="btn-primary inline-flex items-center gap-2" href="/admin/eventos/nuevo"><Plus className="size-4" />Crear evento nuevo</a></div></div>
    {error && <p className="rounded-lg bg-danger-50 p-3 text-danger-700">{error}</p>}
    <EventSection title="Eventos activos" empty="No hay eventos recibiendo inscripciones.">{active.map(event => <EventCard key={event.id} event={event} action={<button className="btn-secondary text-sm" onClick={() => setStatus(event.id, 'closed')}>Cerrar inscripciones</button>} />)}</EventSection>
    <EventSection title="Próximos eventos" empty="No hay borradores pendientes.">{upcoming.map(event => <EventCard key={event.id} event={event} action={<button className="btn-primary text-sm" onClick={() => setStatus(event.id, 'active')}>Activar</button>} />)}</EventSection>
    <section className="card overflow-hidden"><button className="flex w-full items-center justify-between p-5 text-left" onClick={() => setPastOpen(!pastOpen)}><div><h2 className="text-xl font-bold">Eventos pasados</h2><p className="text-sm text-slate-500">{past.length} eventos cerrados o archivados</p></div><ChevronDown className={`transition ${pastOpen ? 'rotate-180' : ''}`} /></button>{pastOpen && <div className="border-t">{past.length ? past.map(event => <div key={event.id} className="flex flex-wrap items-center gap-3 border-b p-4 last:border-0"><div className="min-w-64 flex-1"><p className="font-bold">{event.name}</p><p className="text-sm text-slate-500">{formatDate(event.date_start)} · {event.progress.athletes} nadadores</p></div><a className="btn-secondary text-sm" href={`/admin/eventos/${event.id}`}>Ver detalle</a><a className="btn-secondary inline-flex items-center gap-2 text-sm" href={`/admin/eventos/clonar/${event.id}`}><Copy className="size-4" />Clonar</a></div>) : <p className="p-5 text-slate-500">Todavía no hay eventos pasados.</p>}</div>}</section>
  </main></>
}
