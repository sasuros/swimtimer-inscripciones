...
function EventCard({ event, action }) { const progress = event.progress.clubs ? Math.round(event.progress.received / event.progress.clubs * 100) : 0; return <article className="card p-5"><div className="flex items-start justify-between gap-3"><div><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusInfo[event.status][1]}`}>{statusInfo[event.status][0]}</span><h3 className="mt-3 text-lg font-bold text-brand-800"><a href={`/admin/eventos/${event.id}`}>{event.name}</a></h3></div>{action}</div><p className="mt-3 flex items-center gap-2 text-sm text-slate-500"><CalendarDays className="size-4" />{formatDate(event.date_start)}</p><p className="mt-1 flex items-center gap-2 text-sm text-slate-500"><MapPin className="size-4" />{event.venue || 'Sede por definir'}</p><div className="mt-5 flex items-center justify-between text-xs"><span>{event.progress.received} de {event.progress.clubs} clubes</span><span>{progress}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-brand-600" style={{ width: `${progress}%` }} /></div></article> }
