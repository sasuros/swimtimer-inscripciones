...
export default function AdminEvents() {
  ...
  return <><AdminHeader /><main className="mx-auto max-w-7xl space-y-8 p-4 sm:p-6">
    ...
    <EventSection title="Próximos eventos" empty="No hay borradores pendientes.">{upcoming.map(event => <EventCard key={event.id} event={event} action={<div className="flex gap-2"><button className="btn-secondary text-sm" onClick={() => setStatus(event.id, 'active')}>Activar</button>{deleteButton(event)}</div>} qrAction={qrButton(event)} />)}</EventSection>
