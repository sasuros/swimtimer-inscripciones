...
export default function AdminEvents() {
  ...
  return <><AdminHeader /><main className="mx-auto max-w-7xl space-y-8 p-4 sm:p-6">
    ...
    <EventSection title="Eventos activos" empty="No hay eventos recibiendo inscripciones.">{active.map(event => <EventCard key={event.id} event={event} action={<a className="btn-secondary text-sm" href={`/admin/eventos/${event.id}`}>Gestionar</a>} qrAction={qrButton(event)} />)}</EventSection>
