import { shortenAthlete } from './shortName'
...
export async function buildConsolidatedExport({ event, inscriptions = [], lateInscriptions = [], type = 'principal' }) {
  ...
  const addSubmission = (submission, late) => {
    ...
    selectedAthletes.forEach(athlete => {
      ...
      athletes.push(normalizeAthlete(shortenAthlete({ ...athlete, Ath_no: newId, Comp_no: newId }), late))
    })
...
// JSON por club para Meet Manager: athletes con nombre corto; roster queda completo.
export async function buildClubFileExport(inscription) {
  const output = { ...inscription, meta: { ...inscription.meta, sha256: '' }, athletes: (inscription.athletes || []).map(shortenAthlete) }
  output.meta.sha256 = await sha256(output)
  return output
}
