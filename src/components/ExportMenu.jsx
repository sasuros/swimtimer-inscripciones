...
export default function ExportMenu({ onExport }) {
  ...
  return <div className="relative"><button className="btn-secondary inline-flex items-center gap-2 text-sm" onClick={() => setOpen(!open)}><Download className="size-4" />Descargar JSON <ChevronDown className="size-3" /></button>{open && <div className="absolute right-0 z-20 mt-2 w-64 overflow-hidden rounded-xl border bg-[#0f1f3a]"><Option label="Consolidado principal" detail="Solo inscripciones normales" onClick={() => { onExport('principal'); setOpen(false) }} /><Option label="Consolidado completo" detail="Normales + tardías aprobadas" onClick={() => { onExport('completo'); setOpen(false) }} /><Option label="Suplemento tardías" detail="Solo tardías aprobadas" onClick={() => { onExport('supplement'); setOpen(false) }} /></div>}</div>
}
