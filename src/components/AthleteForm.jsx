...
export default function AthleteForm({ roster, referenceDate, eventConfig, editing, onSave, onCancelEdit, onQuickImport }) {
  ...
  return <form onSubmit={submit} className="card p-4 sm:p-6">
    ...
    <section className="space-y-4"><StepTitle number="1" title="Datos del nadador" /><div className="grid gap-4 sm:grid-cols-2">
      ...
      <div><label className="label" htmlFor="birthDate">Fecha de nacimiento *</label><input id="birthDate" type="date" min="2007-01-01" max="2022-12-31" className={fieldClass('birthDate')} value={form.birthDate} onChange={e => { setInteracted(true); setForm(current => ({ ...current, birthDate: e.target.value, selectedEvents: [], times: {} })) }} /><p className="field-help">La edad se calcula con la fecha de referencia del evento</p><ErrorMessage>{showErrors && errors.birthDate}</ErrorMessage></div>
    </div>
    {category && form.sex && <div className={`rounded-xl p-4 text-center text-lg font-extrabold ${form.sex === 'F' ? 'bg-female-50 text-female-800' : 'bg-male-50 text-male-800'}`}>{category.label} · {form.sex === 'F' ? 'Femenino' : 'Masculino'}<span className="mt-1 block text-xs font-semibold opacity-70">Edad calculada: {age} años</span></div>}
    ...
    {form.selectedEvents.length > 0 && <section className="mt-8 space-y-4"><StepTitle number="3" title="Tiempos de inscripción" />{form.selectedEvents.map(index => <TimeInput key={index} event={availableEvents.find(item => item.eventIndex === index)} value={form.times[index]} showError={showErrors} onChange={value => { setInteracted(true); setForm(current => ({ ...current, times: { ...current.times, [index]: value } })) }} />)}</section>}
